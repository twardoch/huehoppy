# huehoppy.algorithms.colortrans_pccm - Wrapper for PCCM color transfer (colortrans)
...
try:
    ...
    print(f"Info: Could not import 'colortrans' submodule. PCCM algorithm may not be available. Error: {e}")
    colortrans = None

from huehoppy.core.base_algorithm import ColorTransferAlgorithm

class ColortransPCCM(ColorTransferAlgorithm):
    """
    Principal Components Color Matching (PCCM) Color Transfer.

    Wraps the `transfer_pccm` method from the 'colortrans' library by dstein64.
    Based on: Kotera et al. (1998, 2005).
    ...
    def __init__(self):
        if colortrans is None:
            raise ImportError(
                "ColortransPCCM algorithm requires 'colortrans' which could not be imported. "
    ...
    def transfer(self, source_image: np.ndarray, target_image: np.ndarray) -> np.ndarray:
        ...
        transferred_rgb = colortrans.transfer_pccm(content=target_rgb, reference=source_rgb)
    ...
    def __str__(self) -> str:
        return "PCCM (colortrans)"

    def __repr__(self) -> str:
        return "<ColortransPCCM Algorithm (via colortrans)>"
